...
class MinHash(object):
    ...
    def update(self, b):
        ...
        hv = struct.unpack('<I', xxhash.xxh64(b, seed=self.seed).digest()[:4])[0]
#         hv = struct.unpack('<I', self.hashobj(b).digest()[:4])[0]
    ...
    def deserialize(cls, buf):
        ...
        try:
            ...
            seed, num_perm = struct.unpack_from('qi', buffer(buf), 0)  # @UndefinedVariable
        offset = struct.calcsize('qi')
        try:
            hashvalues = struct.unpack_from('%dI' % num_perm, buf, offset)
        except TypeError:
            hashvalues = struct.unpack_from('%dI' % num_perm, buffer(buf), offset) # @UndefinedVariable
